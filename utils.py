...
def train(config, model, train_loader, dev_loader):
    ...
    for epoch in range(config.num_epochs):
        ...
        for trains in train_loader:
            datas = {key: val.to(config.device) for key, val in trains.items()}
            outputs = model(datas)
            # print(datas['labels'])
            # print(datas['input_ids'].is_cuda, datas['attention_mask'].is_cuda, datas['labels'].is_cuda, outputs.is_cuda)

            model.zero_grad()
            loss = F.cross_entropy(outputs, datas['labels'])
            loss.backward()
            optimizer.step()
            if total_batch % 100 == 0:
                # 每多少轮输出在训练集和验证集上的效果
                true = datas['labels'].data.cpu()
                predic = torch.max(outputs.data, 1)[1].cpu()
                train_acc = metrics.accuracy_score(true, predic)
                dev_acc, dev_loss = evaluate(config, model, dev_loader)
                if dev_loss < dev_best_loss:
                    dev_best_loss = dev_loss
                    torch.save(model.state_dict(), config.save_path)    # 保存权重文件
                    improve = '*'
...
def evaluate(config, model, data_loader, test=False):
    ...
    with torch.no_grad():
        for datas in data_loader:
            datas = {key: val.to(config.device) for key, val in datas.items()}
            outputs = model(datas)
            loss = F.cross_entropy(outputs, datas['labels'])
            loss_total += loss
            labels = datas['labels'].data.cpu().numpy()
